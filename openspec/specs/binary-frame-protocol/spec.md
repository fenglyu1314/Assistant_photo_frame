...
### Requirement: 心跳检测
系统必须支持 PING/PONG 心跳机制，用于桌面端检测墨水屏连接状态。
...
### Requirement: 桌面端 CRC-16/CCITT 计算
桌面端必须实现 CRC-16/CCITT 算法（poly=0x1021, init=0x0000），与固件端 `crc16.cpp` 的实现完全一致。
...
### Requirement: 桌面端二进制帧构建
桌面端必须能构建符合 `protocol.h` 定义的完整二进制帧：MAGIC(2B) + CMD(1B) + LENGTH(4B LE) + DATA(NB) + CRC(2B LE)。CRC 覆盖 CMD + LENGTH + DATA。
...
### Requirement: 桌面端分块传输流程
桌面端必须实现完整的 192KB 帧缓冲区分块传输流程：BEGIN → DATA×N → END，每块等待 ACK 确认。
...
### Requirement: 桌面端 PING/PONG 心跳
桌面端必须支持发送 PING 帧并等待 PONG 响应，用于检测墨水屏连接活性。
...
### Requirement: 桌面端协议常量定义
桌面端模块必须导出与固件 `protocol.h` 完全对齐的协议常量。
