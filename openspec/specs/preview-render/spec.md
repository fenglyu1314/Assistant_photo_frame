...
### Requirement: 独立设备同步
RenderPipeline SHALL provide a `syncToDevice()` method that将已缓存的帧缓冲区发送到设备（仅 Stage 6），不重新渲染。

#### Scenario: 同步已缓存的帧缓冲区
- **WHEN** 调用 `syncToDevice()` 且存在已缓存的帧缓冲区
- **THEN** 必须将缓存的帧缓冲区通过 `SerialManager.sendFrameBuffer()` 发送到设备，返回 `{ success: true, durationMs }`
