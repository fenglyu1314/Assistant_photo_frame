...
RenderPipeline 必须编排从数据收集到墨水屏刷屏的完整流程。

#### Scenario: 一键执行完整管线
- **WHEN** 调用 `RenderPipeline.execute()`
- **THEN** 必须按顺序执行：数据收集 → 离屏渲染 → 截屏 → 饱和度增强 → Floyd-Steinberg 量化 → 物理缓冲区编码 → 串口发送，返回 `{ success, error?, durationMs? }`

#### Scenario: 无设备连接时执行
- **WHEN** 执行管线但串口未连接
- **THEN** 必须在发送阶段返回错误 `{ success: false, error: 'Not connected' }`，前面的渲染和量化步骤正常完成
