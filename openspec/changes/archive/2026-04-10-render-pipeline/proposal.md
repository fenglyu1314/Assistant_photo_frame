...
Phase 1-5 已完成"桩到桩"基础设施：固件接收帧缓冲区并刷屏、桌面端骨架、量化引擎、串口通信。但目前没有任何东西生成帧缓冲区的内容——App.vue 只是一个占位欢迎页，缺少从"数据 → 渲染 → 截屏 → 量化 → 编码 → 发送"的完整管线。Phase 6 的任务是把这条管线跑通，让墨水屏首次显示真实的仪表盘画面。
