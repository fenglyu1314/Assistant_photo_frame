...
本设计为固件添加二进制帧协议处理，使 ESP32 能从 PC 端接收 192KB 帧缓冲区数据并驱动墨水屏刷新。这是渲染管线的基础——没有通信能力，后续所有 Phase（Electron 串口通信、渲染管线）都无法推进。
...
**选择**：逐块 ACK/NAK，PC 端等 ACK 后再发下一块
...
- PC 端通过 MAGIC 头区分协议帧和调试文本（MAGIC 的 `0xEB` 极少出现在普通文本中）
...
- **缓解**：调试日志使用 ASCII 文本，MAGIC 序列在 ASCII 中极罕见；PC 端严格按 MAGIC + 帧结构解析，非 MAGIC 开头的数据忽略
