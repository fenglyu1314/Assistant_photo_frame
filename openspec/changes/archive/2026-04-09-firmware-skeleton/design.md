...
**理由**：与官方驱动和 Companion 端的量化引擎保持一致。4-bit packed 格式下 800×480 的帧缓冲区为 192,000 字节，刚好适配 PSRAM。官方驱动的 `EPD_Sendbuffera` 直接发送 packed 数据到屏幕。
...
**理由**：10 MHz 更保守稳定。Phase 1 的目标是验证能工作，后续可调高。完整版用 40 MHz 是因为要快速加载大图，而我们的场景由 PC 端推送，刷屏速度不是首要关注点。
